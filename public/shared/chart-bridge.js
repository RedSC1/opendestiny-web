...
export function restoreChart(form,calculate,kind){
 ...
 try{const record=readRecord(id);if(!record||!Array.isArray(record.form))throw new Error('找不到这份命例');
  ...
  apply();for(const name of ['inputCalendar','clockMode','lunarLeapToggle','lunarSpecialToggle','calendarMode']){const el=form.querySelector(`[name="${name}"]:checked`)||form.querySelector(`[name="${name}"]`);el?.dispatchEvent(new Event('change',{bubbles:true}));}apply();
